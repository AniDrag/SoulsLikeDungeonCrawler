...
public class AICore_Sense : MonoBehaviour
{
    ...
    public LayerMask targetLayers;      // Layers that contain entities (Player, Enemies, etc.)
    public LayerMask obstacleLayers;    // Layers that block sight (Walls, Terrain, etc.)
    ...
    public void Core_Update()
    {
        ...
        for (int i = 0; i < numFound; i++)
        {
            ...
            // Check if target is within the view cone
            float angleToTarget = Vector3.Angle(transform.forward, directionToTarget);
            bool inViewCone = angleToTarget < viewAngle / 2;

            if (inViewCone)
            {
                if (!Physics.Raycast(transform.position, directionToTarget, out RaycastHit hit, distanceToTarget, obstacleLayers))
                {
                    detectedTargets.Add(target);
#if UNITY_EDITOR
                    Debug.DrawLine(transform.position, target.transform.position, Color.green, 0.1f);
#endif
                }
                else
                {
